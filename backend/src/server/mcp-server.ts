...
async function main() {
  try {
    // Validar instalación de Foundry al iniciar el servidor
    await validateFoundryInstallation()
    
    const transport = new StdioServerTransport()
    await server.server.connect(transport)
    // Usar console.error para logs en stderr (no interfiere con STDIO)
    console.error('Servidor MCP Foundry Tools iniciado (STDIO)')
    // El servidor debe mantener el proceso corriendo
    // No llamar process.exit() aquí - el servidor debe mantener el proceso vivo
  } catch (error) {
