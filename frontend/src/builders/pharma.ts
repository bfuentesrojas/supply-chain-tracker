...
export function buildBomFeatures(input: BomBuilderInput): BuilderResult<BomFeatures> {
  ...
  if (validation.success) {
    ...
    // NOTA: Los parentIds ahora se manejan en el contrato inteligente (parentIds y parentAmounts),
    // por lo que NO incluimos la sección 'parents' en el JSON de features
    const schemaJson = {
      ...
      custom: {
        ...
        // Los componentes ahora son opcionales - se manejan como parentIds en el contrato
        // Si hay componentes, incluirlos para referencia (aunque normalmente estarán vacíos)
        ...(input.components && input.components.length > 0 && {
          components: input.components.map(comp => ({
            ...
            ...(comp.percentage && { percentage: comp.percentage })
          }))
        }),
...
export function buildSsccFeatures(input: SsccBuilderInput): BuilderResult<SsccFeatures> {
  const features: SsccFeatures = {
    ...
    // contents ya no se incluye - el contrato maneja los padres (parentIds y parentAmounts)
    logistics: {
  ...
  if (validation.success) {
    // Transformar al formato del schema JSON antes de serializar
    // NOTA: Los parentIds ahora se manejan en el contrato inteligente (parentIds y parentAmounts),
    // por lo que NO incluimos la sección 'contents' en el JSON de features
    const { contents, ...schemaJson } = features
