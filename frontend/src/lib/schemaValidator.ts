...
function validateTypeSpecificFields(
  ...
  switch (type) {
    ...
    case 'BOM':
      ...
      // por lo que NO validamos la sección 'parents' en el JSON de features.
      // Los componentes ya no son requeridos en el JSON - se manejan como parentIds en el contrato.
      // custom.components es opcional y solo para referencia si se incluye.
      break
    ...
    case 'SSCC':
      ...
      // contents ya no se valida - el contrato maneja los padres (parentIds y parentAmounts)
      if (obj.gs1) {
        const gs1 = obj.gs1 as Record<string, unknown>
