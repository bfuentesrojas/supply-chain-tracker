...
export function useSupplyChain() {
  ...
  // Obtener jerarquía completa de un token (hasta el root) agrupada por niveles
  const getTokenHierarchy = useCallback(async (tokenId: bigint): Promise<Token[][]> => {
    ...
    try {
      ...
      while (currentLevel.length > 0) {
        ...
        // Agregar todos los tokens del nivel actual como un nivel completo
        if (currentLevelTokens.length > 0) {
